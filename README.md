...
3. Place the project in your web root (e.g. `C:\xampp\htdocs\notes`). The included `.htaccess` files route all `/notes/*` requests to the front controller in `public/index.php`, so the app is available at `http://localhost/notes`.
