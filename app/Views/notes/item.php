...
<div class="note <?= strtolower($note['status']); ?>">
    ...
    <?php endif; ?>
    <form method="post" action="<?= BASE_PATH; ?>/notes/<?= $note['id']; ?>/update">
        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token); ?>">
        <textarea name="content" rows="3" cols="40"><?= htmlspecialchars($note['content']); ?></textarea>
        <button type="submit">Update</button>
        <?php if ($user['role'] === 'admin'): ?>
            <button formaction="<?= BASE_PATH; ?>/notes/<?= $note['id']; ?>/delete" formmethod="post" onclick="return confirm('Delete?')">Delete</button>
        <?php endif; ?>
    </form>
    <div>
        Status:
